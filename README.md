...
The window is a small native shell (Dutch menus and toolbar) with a Chromium editor inside. It is meant to be used as `MarkdownViewer.exe`, not as a website.

## What you can do

- Open, save, and save-as markdown (`.md` / `.markdown`)
- Edit the rendered document: bold, headings, links, lists, a horizontal rule (**Streep**), and a task list (**Taken**)
...
- Work with several documents in tabs. **Nieuw** opens an empty tab; **Openen** loads into the current tab. At least one tab stays open. Closing the window asks about every unsaved tab.
...
These cover file load/save and resolving local markdown links.

## How it is put together

| Part | Role |
| --- | --- |
| `src/` | Typhon desktop shell: window, tabs, document, link resolution |
...
| `tests/` | File and link unit tests |
